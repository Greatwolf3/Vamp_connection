...
@endif
    <form method="post" action="{{ route("save_contatto_generico") }}">
        <input name="_token" value="{{ csrf_token() }}" type="hidden">
        <input type="hidden" name="id_pg" id="id_pg" value="{{session()->get("id_pg")}}">
        <div class="row">

            <div class="col-3">
                <label for="nome_contatto" class="label_size">{{ trans('user.nome_contatto')  }}</label>
                <input type="text" id="nome_contatto" name="nome_contatto" value="">
                @if ($errors->has('nome_contatto')) <p class="help-block" style="color:red;">
                    {{ $errors->first('nome_contatto') }}</p>
                @endif
